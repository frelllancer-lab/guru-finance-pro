...
app.post('/api/ai/advice', async (req, res) => {
  try {
    ...
    const promptText = Ты - финансовый консультант. Период: , Валюта: , Доходы: , Расходы: , Долг: , Счетов: . Дай 3 совета НА .;
...
app.post('/api/ai/chat', async (req, res) => {
  try {
    ...
    const def = { uk: 'Вітаю! Я ваш фінансовий менеджер.', en: 'Hello! I am your financial manager.', ru: 'Привет! Я твой финансовый менеджер.' };
    const ai = getGeminiClient();
    if (!ai) return res.json({ success: true, reply: def[lang] });
    const ctx = userFinancialContext ? Баланс: , Доходы: , Расходы: . : '';
    const li = { uk: 'Відповідай УКРАЇНСЬКОЮ!', en: 'Respond in ENGLISH!', ru: 'Отвечай на РУССКОМ!' };
    const hist = (messages || []).map((m: any) => ${m.sender === 'user' ? 'User' : 'Manager'}: ).join('\n\n');
    const prompt = Ты финансовый менеджер.   История:\n\nОтвет:;
...
app.post('/api/ai/scan-receipt', async (req, res) => {
  try {
    ...
    const promptText = Проанализируй кассовый чек. Извлеки: type (expense/income), amount (число), currency (UAH/USD), category из [food, transport, shopping, home, fun, health, services, cash, other], note (название), date (ISO).;
...
app.post('/api/ai/scan-multibank', async (req, res) => {
  try {
    ...
    const promptText = Проанализируй скриншоты банков. Найди accounts (bank, name, ownBalance, debt, minPayment, currency) и transactions (type, amount, currency, category из [food, transport, shopping, home, fun, health, services, cash, salary, freelance, gift, invest, other], note, date).;
    const response = await ai.models.generateContent({
      model: 'gemini-2.5-flash',
...
app.post('/api/ai/scan-pdf', async (req, res) => {
  try {
    ...
    const promptText = Проанализируй текст банковской выписки. Извлеки accounts (bank, name, ownBalance, debt, minPayment, currency) и transactions (type, amount, currency, category из [food, transport, shopping, home, fun, health, services, cash, salary, freelance, gift, invest, other], note, date в ISO). Текст: ;
    const response = await ai.models.generateContent({
      model: 'gemini-2.5-flash',
      contents: promptText,
      config: {
...
app.post('/api/bank/monobank/sync', async (req, res) => {
  try {
    ...
    const parsed = (clientInfo.accounts || []).map((acc: any) => {
      ...
      const bal = (acc.balance || 0) / 100, cl = (acc.creditLimit || 0) / 100;
      let own = bal >= 0 ? bal : 0, debt = bal < 0 ? Math.abs(bal) : 0;
      ...
      return { id: 'mono_' + acc.id, bank: 'Monobank', name: ${tl}, ownBalance: Math.round(own * 100) / 100, debt: Math.round(debt * 100) / 100, minPayment: debt > 0 ? Math.round(debt * 0.05 * 100) / 100 : 0, currency: cur };
    });
  ...
  try {
    ...
    if (!ai) {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (cols.length >= 3) { const a = parseFloat(cols[cols.length - 1] || cols[cols.length - 2]); if (!isNaN(a)) txs.push({ type: a < 0 ? 'expense' : 'income', amount: Math.abs(a), currency: 'USD', category: 'shopping', note: cols[1] || 'Apple Card', date: new Date().toISOString() }); }
      }
...
app.post('/api/sync/apple-wallet-shortcut', (req, res) => {
  ...
  } catch (err: any) { res.status(400).json({ error: 'Invalid data' }); }
});
